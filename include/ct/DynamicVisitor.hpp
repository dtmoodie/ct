...
namespace ct
{
    ...
    template <class T>
    void IDynamicVisitor::pushCach(T&& val, const std::string& name)
    {
    ...
    template <class T>
    bool IDynamicVisitor::tryPopCache(T& val, const std::string& name)
    {
        auto& container = accessCache(name);
        if (container)
        {
    ...
    template <class T>
    T IDynamicVisitor::popCache(const std::string& name)
    {
        T ret;
