...
namespace ct
{
    ...
    struct TTraits<std::vector<T>, void> : public IContainerTraits
    {
        ...
        TTraits(std::vector<T>* vec) : m_vec(vec) {}
        ...
        virtual size_t numValues() const override { return m_vec->size(); }
        void setKeys(const size_t) {}
        void setValues(const size_t num) { m_vec->resize(num); }
      private:
        std::vector<T>* m_vec;
    };

    template <class T>
    struct TTraits<const std::vector<T>, void> : public IContainerTraits
    {
        using base = IContainerTraits;

        TTraits(const std::vector<T>* vec) : m_vec(vec) {}
