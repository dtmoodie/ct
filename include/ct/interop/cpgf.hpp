...
namespace ct
{
    ...
    template <class T, class DEFINITION, index_t I>
    typename std::enable_if<std::is_same<typename AccessorType<T, I>::GetterType_t, MemberAccessorGetterType>::value &&
                            !std::is_same<typename AccessorType<T, I>::SetterType_t, void>::value>::type
    registerHelperImpl(DEFINITION& def, const Indexer<I> idx)
    {
        auto accessor = Reflect<T>::getPtr(idx);
    ...
    auto createDefinition(const char* name, VariadicTypedef<BASES...>)
        -> decltype(cpgf::GDefineMetaClass<T, BASES...>::define(name))
    {
