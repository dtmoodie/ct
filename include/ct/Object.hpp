...
#define DECLARE_CLASS_HASH                                                                                             \
    static CTCONSTEXPR uint32_t getHash() { return ct::hashClassName(CT_STRUCT_MAGIC_FUNCTION); }

#define DECLARE_MODULE_HASH(N)                                                                                         \
    static CTCONSTEXPR uint32_t getHash() { return ct::crc32(CT_STRUCT_MAGIC_FUNCTION) ^ N; }                          \
    enum : uint32_t                                                                                                    \
    {                                                                                                                  \
        hash = getHash()                                                                                               \
    };
...
namespace ct
{
    ...
    struct HashedObject<T, N, typename std::enable_if<std::is_pod<remove_reference_t<T>>::value>::type>
    {
        ...
        HashedObject(T&& value_) : value(std::forward<T>(value_)) {}
    ...
    inline HashedObject<T, N> makeHashed(T&& value)
    {
