...
namespace ct
{
    namespace ext
    {
        ...
        struct IDataTableBase
        {
            ...
            T* begin(T U::*mem_ptr)
            {
                static_assert(std::is_same<U, DTYPE>::value || IsBase<Base<DTYPE>, Derived<U>>::value, "Mem ptr must derive from DTYPE");
            ...
            void populateData(DTYPE out, size_t idx)
            {
                populateDataRecurse(out, idx, Reflect<DTYPE>::end());
            }
        private:
            ...
            void populateDataRecurse(DTYPE out, size_t row, Indexer<0> idx)
            {
