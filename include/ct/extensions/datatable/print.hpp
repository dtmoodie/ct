...
namespace ct
{
    namespace ext
    {
        ...
        void printTableElement(std::ostream& os,
                               const ext::IDataTable<T>& table,
                               size_t i,
                               Indexer<I>,
                               MemberObjectPointer<TArrayView<DTYPE> U::*, FLAGS, MDATA> ptr)
        {
            auto tensor = table.begin(ptr.m_ptr);
            if (I != 0)
            {
                os << ", ";
            }
